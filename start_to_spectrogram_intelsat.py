...
def start_to_spectrogram_intelsat(infilename, cpi, overlap_factor, telescope, channel, window_function, padding):
    ...
    rcm_map, spectrogram, peak_history, number_of_strips = pc_and_spectrogram(target_name, height, points, overlap_factor, iq_samples, samp_rate, pri, alpha, intelsat_tle, telescope, Tp, freq, padding, window_function)
    print('\nProcessing Complete.')
    ...
    plotter(spectrogram, "Micro-Doppler Spectrogram", "CPI Index", "Doppler Velocity (m/s)", [0, spectrogram.shape[1], -max_velocity, max_velocity], target_name, height, overlap_factor, channel, window_function, padding)
...
def start_to_spectrogram_atlas_function(infilename, cpi, overlap_factor, telescope, channel, window_function, padding):
    ...
    rcm_map, spectrogram, peak_history, number_of_strips = pc_and_spectrogram(target_name, height, points, overlap_factor, iq_samples, samp_rate, pri, alpha, atlas_tle, telescope, Tp, freq, padding, window_function)
    print('\nProcessing Complete.')
    ...
    plotter(spectrogram, "Spectrogram", "CPI Index", "Doppler Velocity (m/s)", [0, spectrogram.shape[1], -max_velocity, max_velocity], target_name, height, overlap_factor, channel, window_function, padding)
    ...
    np.save('./spectrogram_' + target_name + '_' + str(height) + '_'+str(height//overlap_factor)+'channel_'+str(channel)+str(window_function)+str(padding)+'.npy', spectrogram)
    return
